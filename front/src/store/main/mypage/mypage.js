...
const actions = {
  ...
  actOrderList({ commit }, value) {
    axios({
      url: "http://localhost:6666/order/member_list",
      method: "get",
  ...
  actWriteReview({ getters }, value) {
    ...
    axios
      .post(
        `http://localhost:6666/${value}/${orderInfo.market_name}/${orderInfo.orderId}`,
        formData,
        {
          withCredentials: true,
      ...
      .then((res) => {
        if (res.data.message) alert("작성이 완료 되었습니다.");
      })
      .catch((err) => console.log(err));
  },
  actReserveList({ commit }, value) {
    axios({
      url: "http://localhost:6666/reservation/myReserve",
      method: "get",
      withCredentials: true,
    }).then(async (res) => {
  ...
  actChangeMemberInfo({ commit }, value) {
    if (value.nicknameCheck === false && value.nickname !== null)
    ...
    else if (value.nickname === "") alert("닉네임을 적어주세요");
    else {
      let formData = new FormData();
      try {
      ...
      axios
        .post(
          "http://localhost:6666/users/memberInformation/update",
          formData,
          {
            withCredentials: true,
            headers: { "Content-Type": "multipart/form-data" },
  ...
  checkNickName({ getters }, value) {
    if (getters.getChangeData.nickname !== null)
      axios({
        url: `http://localhost:6666/users/checkNickName/${getters.getChangeData.nickname}`,
        method: "get",
        withCredentials: true,
      }).then(async (res) => {
        getters.getChangeData.nicknameCheck = res.data;
        if (res.data === true) alert("사용 가능한 닉네임 입니다");
  ...
  checkMarketName({ getters }, value) {
    if (getters.getChangeData.nickname !== null)
      axios({
        url: `http://localhost:6666/users/checkMarketName/${getters.getChangeData.nickname}`,
        method: "get",
        withCredentials: true,
      }).then(async (res) => {
        getters.getChangeData.nicknameCheck = res.data;
  ...
  actChangeMarketInfo({ commit }, value) {
    ...
    else if (value.nickname === "") alert("가게 이름을 적어주세요");
    else {
      axios({
        method: "post",
        url: "http://localhost:6666/users/marketInformation/update",
        withCredentials: true,
        data: {

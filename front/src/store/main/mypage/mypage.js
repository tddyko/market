...
const actions = {
  ...
  actWriteReview({getters},value){
    ...
    axios.post(`/api/${value}/${orderInfo.market_name}/${orderInfo.orderId}`,formData,{
          withCredentials: true,
  ...
  actChangeMemberInfo({commit},value){
   ...
   formData.append('password',value.password)
    axios.post('/api/users/memberInformation/update',formData,{
      withCredentials : true,
      headers : {'Content-Type': 'multipart/form-data'}
  ...
  if(getters.getChangeData.nickname!==null)
    axios({
      url : `/api/users/checkNickName/${getters.getChangeData.nickname}`,
      method : 'get',
      withCredentials : true
    }).then(async(res)=>{
   ...
   checkMarketName({getters},value){
    if(getters.getChangeData.nickname!==null)
      axios({
        url : `/api/users/checkMarketName/${getters.getChangeData.nickname}`,
        method : 'get',
        withCredentials : true
      }).then(async(res)=>{
        getters.getChangeData.nicknameCheck = res.data
   ...
   actChangeMarketInfo({commit},value){
      ...
      axios({
        method:'post',
        url : '/api/users/marketInformation/update',
        withCredentials : true,
        data : {

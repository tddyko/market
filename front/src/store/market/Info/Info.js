...
const mutations = {
  ...
  Update_Open_Time_Menu(state) {
    if (parseInt(state.Open_Time.toString().substring(0, 2)) > 12) {
      state.Open_Time =
        "오후 " +
        (parseInt(state.Open_Time) - 12) +
        state.Open_Time.toString().substring(2, 5);
    } else if (parseInt(state.Open_Time.toString().substring(0, 2)) < 10) {
      state.Open_Time =
        "오전 " +
        state.Open_Time.toString().substring(1, 2) +
        state.Open_Time.toString().substring(2, 5);
    } else {
  ...
  Update_Close_Time_Menu(state) {
    if (parseInt(state.Close_Time.toString().substring(0, 2)) > 12) {
      state.Close_Time =
        "오후 " +
        (parseInt(state.Close_Time) - 12) +
        state.Close_Time.toString().substring(2, 5);
    } else if (parseInt(state.Close_Time.toString().substring(0, 2)) < 10) {
      state.Close_Time =
        "오전 " +
        state.Close_Time.toString().substring(1, 2) +
        state.Close_Time.toString().substring(2, 5);
    } else {
...
const actions = {
  ...
  send_infor_values({ state }) {
    ...
    if (state.Open_Time.substring(0, 2) == "오전")
      ...
      formData.append(
        "start_time",
        parseInt(state.Open_Time.substring(3)) +
          12 +
          state.Open_Time.substring(5)
      );
    if (state.Close_Time.substring(0, 2) == "오전")
      ...
      formData.append(
        "end_time",
        parseInt(state.Close_Time.substring(3)) +
          12 +
          state.Close_Time.substring(5)
      );
    ...
    axios
      .post("http://localhost:6666/mymarket/update/inform", formData, {
  ...
  sendNotices: ({ state }) => {
    ...
    axios
      .post("http://localhost:6666/mymarket/update/notice", formData, {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then((res) => {})

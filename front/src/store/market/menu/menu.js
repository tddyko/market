...
const mutations = {
  ...
  setDelete:(state,event) => {
     for(let index of state.menu_checkbox){
        ...
        axios({
          url :`/api/menu/delete/${selectMenu.product_id}`,
          method : 'delete',
  ...
  updateMenu:(state,payload) => {
    for(let index of state.menu_checkbox){
      ...
      axios.post(`/api/menu/update/${selectMenu.product_id}`,formData,
      {
        withCredentials: true,
...
const actions = {
  ...
  actMenu:({commit},value)=>{
    axios({
      url:'/api/menu/myMarket/menuList',
      method :'get',
      withCredentials : 'true'
    }).then(async(res)=>{
  ...
  actOptionGroup : ({getters},value)=>{
     ...
     console.log(getters.getMenu_Option[index].name)
           axios({
              url : `/api/menu_option/updateOptionGroup/${getters.getSelectMenuId}/${getters.getMenu_Option[index].pd_option_group_id}`,
              method : 'post',
              withCredentials : true,
                data : {
  ...
  actOption_Select_Name :({commit},value)=>{
    axios({
      url : `/api/menu_option/list/optionGroup/${value}`,
      method : 'get',
      withCredentials : true
    }).then(async(res)=>{
      if(res.data)
  ...
  actMenu_Option :({commit},value)=>{
    axios({
      url : `/api/menu_option/list/GroupAndOptions/${value}`,
      method : 'get',
      withCredentials : true
    }).then(async(res)=>{
      if(res.data)
  ...
  actMenu_Options({getters},value){
    for(let id of getters.getOption_item_checkbox){
     let selectOption = getters.getMenu_Option[value]
        axios({
           url :`/api/menu_option/updateOption/${selectOption.pd_option_group_id}/${selectOption.Pd_options[id].pd_option_id}`,
           method : 'post',
           withCredentials : true,
           data :{
    ...
    for(let item of getters.getDeleteOptions){
      axios({
               url :`/api/menu_option/delete/options/${item.pd_option_id}`,
               method : 'get',
               withCredentials : true,
      }).then(async(res)=>{
      }).catch((err)=>{console.log(err)})
  ...
  actDeleteOption({getters},value){
    for(let index of getters.getOption_Checkbox){
         axios({
             url :`/api/menu_option/delete/option_group/${getters.getMenu_Option[index].pd_option_group_id}`,
             method : 'get',
             withCredentials : true,
           }).then(async(res)=>{
         }).catch((err)=>{console.log(err)})
  ...
  actRoom({commit},value){
    axios({
      url : '/api/market_preview/myMarket/roomlist',
      method : 'get',
      withCredentials : true
    }).then(async(res)=>{
      console.log('acRoom')
  ...
  actRoomDelete({getters},value){
    for(let index of getters.getRoom_Checkbox){
            ...
            axios({
              url :`/api/market_preview/myMarket/deleteRoom/${selectRoom.room_id}`,
              method : 'delete',
              withCredentials : true,
            })
    }
  ...
  actSetRoom({getters},value){
    for(let index of getters.getRoom_Checkbox){
        ...
        axios.post(`/api/market_preview/room/${selectRoom.room_id}`,formData,
           {
              withCredentials: true,
             headers : {'Content-Type': 'multipart/form-data'}
           }).then((res)=>{})

...
const state = () => ({
  signUp: [{ marketSignupDialog: null }, { memberSignupDialog: null }],
  login_switch: false,
  isHaveSession: null,
  userInfo: null,
  marketInfo: null,
...
const actions = {
  ...
  actUserInfo({ commit }, value) {
    axios({
      url: "http://localhost:6666/users/storeInformation",
      method: "get",
      ...
      if (!res.data.message) await commit("setMarketInfo", res.data);
        ...
        axios({
          url: "http://localhost:6666/users/memberInformation",
          method: "get",
          withCredentials: true,
        }).then(async (res) => {
          if (!res.data.message) commit("setUserInfo", res.data);
        });
      }
    });
  },

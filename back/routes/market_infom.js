...
router.get("/:marketNm/totalRating", async (req, res) => {
  ...
  let sum =
    (await Reserve_review.sum("rating", { where: { market_id } })) +
      (await Order_review.sum("rating", { where: { market_id } })) || 0;
  let count =
    (await Reserve_review.count({ where: { market_id } })) +
      (await Order_review.count({ where: { market_id } })) || 0;
  var array = [];
  for (var i = 1; i <= 5; i++)
    array[i - 1] =
      ((((await countRating(Reserve_review, req.params.marketNm, i)) || 0) +
        ((await countRating(Order_review, req.params.marketNm, i)) || 0)) /
        count) *
      100;
...
router.get("/:marketNm/reserve/find", async (req, res) => {
  ...
  for (let i = start_time; i <= end_time; i++)
    daterange.push(
      dayjs().set("hour", i).set("minute", 0).format("YYYY-MM-DD HH:mm")
    );

  let datas = {};
  datas.reservetotalCount = await getValues(Reservation, daterange, "예약");
  datas.reservecancelCount = await getValues(
    Reservation,
    daterange,
    "예약취소"
  );
      ...
      where: {
        [Op.and]: [
          sequelize.where(
            sequelize.fn(
              "concat",
              sequelize.col("reserve_date"),
              " ",
              sequelize.col("reserve_time")
            ),
            {
              [Op.gte]: daterange[i],
...
router.post(
  ...
  async (req, res) => {
    ...
    if (req.file) inputData.room_images = req.file.path.substring(7);

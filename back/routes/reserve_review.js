...
router.post(
  "/:marketname/:reservation_id",
  isLoggedInMember,
  upload.array("reserve_reviewImg", 3),
  async (req, res) => {
...
router.get(
  ...
  async (req, res) => {
    ...
    await Reserve_review.update(
      ...
      .then((result) => {
        if (result) res.redirect("reviews");
      })
      .catch((err) => {
        if (err) {
          console.log("수정실패");
    ...
    if (req.files) {
      await Reserve_review_img.destroy({ where: { reserve_review_id } }).then(
        async () => {
  ...
  let results = await Reserve_review.findAll({
    attributes: [
      ...
      [
        sequelize.fn(
          "date_format",
          sequelize.col("Reserve_review.created_at"),
          "%Y년 %m월 %d일"
        ),
        "created_date",
      ],
    ...
    .then((r) => {
      ...
      r.forEach((element) => {
        //switch(tab){
        //   case 0 :
        if (tab === 0) returnData.push(element);
...
router.delete("/:id", isLoggedInMember, async (req, res) => {
  await Reserve_review.destroy({
    ...
    await Reserve_review_answer.destroy({ where: { reserve_review_answer_id } })
      .then((r) => {
        if (r) console.log("성공");
      })
      .catch((err) => {
...
//localhost/reserve_review/recoment/:reserve_review_id uuid값  리뷰 답글 라우터
router.get("/recoment/:reviewId", isLoggedInMarket, async (req, res) => {
  ...
  let reviewData = {
    ...
    market_id: req.user.market_id,
    member_id,
    reserve_review_id: req.params.reviewId,
  };

...
router.get(
  "/list/GroupAndOptions/:product_id",
  isLoggedInMarket,
  async (req, res) => {
...
async function updateOrCreate(tableName, where, inputData) {
  ...
  if (!findData) {
    if (tableName === "Pd_option") inputData.pd_option_id = uuidv4();
...
router.get(
  "/delete/option_group/:option_group_id",
  isLoggedInMarket,
  async (req, res) => {
    await Pd_option_group.destroy({
      where: { pd_option_group_id: req.params.option_group_id },
      force: true,
    });
  }
);
...
router.get(
  "/delete/options/:pd_option_id",
  isLoggedInMarket,
  async (req, res) => {
    Pd_option.destroy({
      where: { pd_option_id: req.params.pd_option_id },
      force: true,
    });
  }
);

...
router.post("/category", isLoggedInMarket, async (req, res) => {
  await Market.findOne({ where: { market_id: req.user.market_id } }).then(
    async (market) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (req.files.profile_img) {
      for (const file of req.files.profile_img) {
        Market.update(
          { profile_img: files.path },
          { where: { market_id: req.user.market_id } }
        ).then((r) => {
          if (r) console.log("가게 이미지 수정 성공");
        });
      }
    }
    if (req.files.info_img) {
      await Market_inform.findOne({
        where: { market_id: req.user.market_id },
        raw: true,
      }).then(async (r) => {
        await Market_inform_img.destroy({
          where: { market_inform_id: r.market_inform_id },
          force: true,
        });
        for (const file of req.files.info_img) {
    ...
    if (req.body.market_inform_week_holiday) {
      let findInformId = await Market_inform.findOne({
...
async function updateOrCreate(tableName, where, inputData) {
  let findData = await tableName
    .findOne({ where: where })
    .catch((error) => console.log(error));
  if (!findData) {
    if (tableName === "Market_inform") inputData.market_inform_id = uuidv4();
    ...
    return tableName.create(inputData).then((r) => {
      if (r) console.log(tableName.tableName + " 값 생성");
    });
  } else {
    console.log(inputData);
    return tableName.update(inputData, { where: where }).then((r) => {
      if (r) console.log(tableName.tableName + " 값 수정");
    });
  }
}

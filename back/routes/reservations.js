...
router.post("/in/:marketname", isLoggedInMember, async (req, res) => {
  ...
  const { current_state, reserve_seat, order_count, date, reserve_time } =
    req.body;
  const reserve_date = dayjs(date).format("YYYY-MM-DD");
  Reservation.create({
...
router.get("/myReserve", isLoggedInMember, async (req, res) => {
  ...
  let myReserve = await Reservation.findAll({
    ...
    r.forEach((element) => {
      element.dataValues.DateAndTime = dayjs(element.DateAndTime).format(
        "MM/DD (ddd) a hh:mm"
      );
    });
    return r;
...
router.get("/list", isLoggedInMarket, async (req, res) => {
  ...
  let reservations = await Reservation.findAll({
    ...
    r.forEach((elements) => {
      elements.reserve_date = dayjs(
        elements.reserve_date + elements.reserve_time
      ).format("YYYY.MM.DD.(ddd)");
      elements.reserve_time = dayjs(
        elements.reserve_date + elements.reserve_time
      ).format("a hh시 mm분");
    });
    return r;
